...
export async function GET(_request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    console.error('[product-option-sets] unexpected error', error);
    return NextResponse.json({ error: 'Unexpected error while loading option set links' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest, { params }: RouteParams) {
  ...
  try {
    ...
    if (setError) {
      console.error('[product-option-sets] failed verifying set', setError);
      return NextResponse.json({ error: 'Failed to verify option set' }, { status: 500 });
    }
    if (!setCheck) {
    ...
    if (typeof displayOrderInput === 'number' && Number.isFinite(displayOrderInput)) {
      ...
      const { data: existing, error: existingError } = await supabase
        ...
        .limit(1);
      if (existingError) {
        console.warn('[product-option-sets] failed fetching display order', existingError);
      }
      const currentMax = existing && existing.length > 0 ? Number(existing[0].display_order ?? 0) : -1;
    ...
    if (error) {
      console.error('[product-option-sets] failed attaching set', error);
      const message = error.code === '23505'
        ? 'This option set is already attached to the product'
        : 'Failed to attach option set';
    ...
    return NextResponse.json({
      link: {
        ...
        alias_label: data.alias_label ?? null,
      },
    }, { status: 201 });
  } catch (error) {
    console.error('[product-option-sets] unexpected attach error', error);
    return NextResponse.json({ error: 'Unexpected error while attaching option set' }, { status: 500 });
  }
}
