...
export async function GET(request: NextRequest, context: { params: Promise<RouteParams> }) {
  ...
  try {
    ...
    try {
      ...
      const seedComponentRefs: { componentId?: number | null; supplierComponentId?: number | null }[] = [];
      ...
      for (const link of productSets ?? []) {
        ...
        for (const group of groups) {
          for (const value of group.values ?? []) {
            ...
            // Collect component refs for org-scoped validation
            if (value.default_component_id != null || value.default_supplier_component_id != null) {
              seedComponentRefs.push({
                componentId: value.default_component_id != null ? Number(value.default_component_id) : null,
                supplierComponentId: value.default_supplier_component_id != null ? Number(value.default_supplier_component_id) : null,
              });
            }
      ...
      // Validate org ownership of all component refs before seeding
...
export async function PATCH(request: NextRequest, context: { params: Promise<RouteParams> }) {
  ...
  try {
    ...
    const replaceSupplierComponentId = payload.replace_supplier_component_id != null
      ? parsePositiveInt(payload.replace_supplier_component_id)
      : null;

    if (replaceComponentId !== null || replaceSupplierComponentId !== null) {
      const refError = await validateOrgScopedComponentRefs(auth.orgId, [{
        componentId: replaceComponentId,
        supplierComponentId: replaceSupplierComponentId,
      }]);
      if (refError) {
