...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const summary = await getLateOrdersSummary(ctx.supabase);
    return NextResponse.json(summary);
  } catch (error) {
    console.error('[assistant][orders][late-summary] Failed to load summary', error);
    return NextResponse.json({ error: 'Failed to load late orders summary' }, { status: 500 });
  }
}
