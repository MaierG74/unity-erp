...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const summary = await getOpenCustomerOrdersSummary(ctx.supabase);
    return NextResponse.json(summary);
  } catch (error) {
    console.error('[assistant][orders][open-summary] Failed to load summary', error);
    return NextResponse.json({ error: 'Failed to load open customer orders summary' }, { status: 500 });
  }
}
