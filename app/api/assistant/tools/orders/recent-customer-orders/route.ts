...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    console.error('[assistant][orders][recent-customer-orders] Failed to load recent customer orders', error);
    return NextResponse.json({ error: 'Failed to load recent customer orders' }, { status: 500 });
  }
}
