-- Extend get_detailed_component_status with reservation columns.
-- Must DROP first because the return type changes (adds reserved_this_order, reserved_by_others).
-- NOTE: shortfall formulas here are the original (pre-fix) versions; the follow-up migration
-- 20260303_fix_shortfall_math_reservation_aware corrects them.
...
WITH
order_components AS (
    SELECT c.component_id, SUM(bom.quantity_required * od.quantity) AS order_required
...
global_requirements AS (
    SELECT c.component_id, SUM(bom.quantity_required * od.quantity) AS total_required, COUNT(DISTINCT od.order_id) AS order_count
    FROM public.order_details od
...
order_details AS (
    SELECT c.component_id,
      jsonb_agg(jsonb_build_object('order_id', od.order_id, 'quantity', bom.quantity_required * od.quantity, 'order_date', o.order_date, 'status', os.status_name)) AS order_breakdown
    FROM public.order_details od
    JOIN public.orders o ON od.order_id = o.order_id
    JOIN public.order_statuses os ON o.status_id = os.status_id
    JOIN public.billofmaterials bom ON od.product_id = bom.product_id
...
supplier_orders AS (
    SELECT sc.component_id,
      jsonb_agg(jsonb_build_object('supplier_order_id', so.order_id, 'supplier_name', s.name, 'quantity', so.order_quantity, 'received', so.total_received, 'status', sos.status_name, 'order_date', so.order_date)) AS on_order_breakdown
...
reservations_this AS (
    ...
    FROM public.component_reservations cr WHERE cr.order_id = p_order_id GROUP BY cr.component_id
),
...
SELECT
    cs.component_id, cs.internal_code, cs.description,
    oc.order_required::INTEGER, gr.total_required::INTEGER, gr.order_count,
    cs.in_stock::INTEGER, cs.allocated_to_orders::INTEGER AS on_order,
...
JOIN public.component_status_mv cs ON oc.component_id = cs.component_id
JOIN global_requirements gr ON oc.component_id = gr.component_id
LEFT JOIN order_details od ON oc.component_id = od.component_id
LEFT JOIN supplier_orders so ON oc.component_id = so.component_id
LEFT JOIN reservations_this rt ON oc.component_id = rt.component_id
LEFT JOIN reservations_others ro ON oc.component_id = ro.component_id;
$$;
