-- Migration: Block allocation rewrites after tracked receipts
-- Date: February 25, 2026
-- Description: Prevent delete-and-reinsert allocation edits once any
-- allocation row on that supplier order has tracked received quantity.

CREATE OR REPLACE FUNCTION public.update_supplier_order_allocations(
    target_supplier_order_id integer,
    new_allocations jsonb,
    target_purchase_order_id integer DEFAULT NULL
) RETURNS void
LANGUAGE plpgsql
SECURITY INVOKER
AS $$
...
BEGIN
    IF EXISTS (
        ...
        WHERE soco.supplier_order_id = target_supplier_order_id
          AND COALESCE(soco.received_quantity, 0) > 0
    ) THEN
        RAISE EXCEPTION 'Cannot reallocate: some allocations have received quantities. Adjust receipts first.';
    END IF;
