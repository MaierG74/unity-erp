-- Fix PL/pgSQL name ambiguity in reserve_order_component_single.
--
-- The function returns columns named component_id/qty_reserved/etc., so
-- unqualified table column references with the same names can be ambiguous
-- inside the function body. Qualify table columns while preserving the
-- original demand calculation and API contract.
