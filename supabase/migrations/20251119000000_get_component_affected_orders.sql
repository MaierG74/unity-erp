...
AS $$
    SELECT 
        o.order_id,
        'ORD-' || o.order_id AS order_number, -- Fallback if order_number column doesn't exist, or use actual column if it does. Checking schema...
        -- Actually, let's check if order_number exists. Based on previous views, it seems we use order_id.
        -- Let's stick to constructing it or selecting it if available.
        -- In get_detailed_component_status, it selects o.order_id.
        -- Let's assume we want to show a friendly ID.
