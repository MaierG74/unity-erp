-- Route factory-floor jobs by top-level category so subcategories inherit the
-- same section as their parent (for example Brackets -> Steel Work).
-- Also tighten the card join to the exact job_card_id encoded in job_instance_id
-- when available, avoiding duplicate floor rows for split-issued sibling cards.

DROP VIEW IF EXISTS public.factory_floor_status;

CREATE VIEW public.factory_floor_status AS
WITH pause_stats AS (
  SELECT
    ape.assignment_id,
    COALESCE(
