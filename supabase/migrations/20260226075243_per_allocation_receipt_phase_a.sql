-- Migration: Per-allocation receipt tracking (Phase A)
-- Date: February 25, 2026
-- Description:
-- 1) Add nullable allocation-level receipt tracking column.
-- 2) Add allocation-aware overload of process_supplier_order_receipt with optional payload.
-- 3) Keep 3-arg callers working via a wrapper function.
...
BEGIN
  ...
  ELSIF v_alloc_count = 1 THEN
    -- Single allocation line: backward-compatible auto-apply.
    UPDATE supplier_order_customer_orders soco
    ...
    END IF;
  ELSIF v_alloc_count > 1 THEN
    -- Phase A mixed-tracking guard: once any allocation on a split line is tracked,
    -- all further receipts must supply an allocation breakdown.
    IF EXISTS (
      SELECT 1
      FROM supplier_order_customer_orders soco
      WHERE soco.supplier_order_id = p_order_id
    ...
    END IF;
    -- Legacy split path in Phase A: leave allocation rows untouched.
  END IF;
  ...
  -- Recompute total_received (now includes this receipt)
...
-- NOTE: 3-arg wrapper was originally here but caused PostgREST ambiguity.
-- Dropped via 20260226091236_drop_receipt_rpc_3arg_wrapper.sql.
-- The 4-arg function with DEFAULT NULL handles callers that omit p_allocation_receipts.
