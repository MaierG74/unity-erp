...
export const processClockEventsIntoSegments = async (dateStr: string): Promise<void> => {
  try {
    ...
    if (eventsError) {
      console.error('Error fetching clock events:', eventsError);
      return;
    }
    ...
    for (const staffId in staffEvents) {
      const events = staffEvents[staffId];
      
      // Find existing segments to avoid duplicates
      const { data: existingSegments, error: segmentsError } = await supabase
        .from('time_segments')
        .select('clock_in_event_id, clock_out_event_id')
        .eq('staff_id', staffId)
        .eq('date_worked', dateStr);
      
      if (segmentsError) {
        console.error('Error fetching existing segments:', segmentsError);
        continue;
      }
      
      // Create sets of processed event IDs
      const processedInEvents = new Set(existingSegments?.map(s => s.clock_in_event_id) || []);
      const processedOutEvents = new Set(existingSegments?.map(s => s.clock_out_event_id) || []);
      
      // Process work segments (clock_in -> clock_out pairs)
      // Treat clock_in/clock_out as work regardless of break_type (including 'work break')
      const clockInEvents = events.filter(e => e.event_type === 'clock_in');
      const clockOutEvents = events.filter(e => e.event_type === 'clock_out');
      // Only use break_start/break_end for breaks, not for work segments.
      
      // First, let's log what we're working with
      console.log(`Staff ${staffId}: ${clockInEvents.length} clock-ins, ${clockOutEvents.length} clock-outs`);
      
      // If there are unpaired events, let's try to pair them
      if (clockInEvents.length > 0 || clockOutEvents.length > 0) {
        ...
        clockOutEvents.sort((a, b) => new Date(a.event_time).getTime() - new Date(b.event_time).getTime());
        
        // If we have more clock-ins than clock-outs, add a virtual clock-out at the end of the day
        if (clockInEvents.length > clockOutEvents.length) {
          const lastClockIn = clockInEvents[clockInEvents.length - 1];
          // Create a virtual clock-out at the end of the day if there's no matching clock-out
          if (!clockOutEvents.some(out => new Date(out.event_time) > new Date(lastClockIn.event_time))) {
            const endOfDay = new Date(`${dateStr}T23:59:59`);
            const durationMins = calculateDurationMinutes(lastClockIn.event_time, endOfDay.toISOString());
            
            // Create work segment with the virtual clock-out
            const { error: insertError } = await supabase
              .from('time_segments')
              .insert({
                staff_id: parseInt(staffId),
                date_worked: dateStr,
                clock_in_event_id: lastClockIn.id,
                clock_out_event_id: null, // Virtual clock-out
                start_time: lastClockIn.event_time,
            ...
            if (insertError) {
              console.error('Error creating virtual work segment:', insertError);
            } else {
              console.log(`Created virtual work segment for staff ${staffId} from ${lastClockIn.event_time} to end of day`);
              processedInEvents.add(lastClockIn.id);
            }
          }
        }
      }
      ...
      for (const clockOutEvent of clockOutEvents) {
        // Skip if this clock_out event is already processed
        if (processedOutEvents.has(clockOutEvent.id)) {
          console.log(`Skipping already processed clock-out event ${clockOutEvent.id}`);
          continue;
        }
        
        // Find the most recent unprocessed clock_in event before this clock_out
        ...
        if (matchingClockIn) {
          ...
          console.log(`Creating work segment for staff ${staffId} from ${matchingClockIn.event_time} to ${clockOutEvent.event_time} (${durationMins} mins)`);
          ...
          if (insertError) {
            console.error('Error creating work segment:', insertError);
            continue;
          }
          ...
          console.log(`No matching clock-in found for clock-out event ${clockOutEvent.id}`);
        }
      }
      
      // Process break segments (break_start -> break_end pairs)
      ...
      for (const breakEndEvent of breakEndEvents) {
        // Skip if this break_end event is already processed
        if (processedOutEvents.has(breakEndEvent.id)) continue;
        
        // Find the most recent unprocessed break_start event before this break_end
        const matchingBreakStart = breakStartEvents
          .filter(e => !processedInEvents.has(e.id) && 
                      new Date(e.event_time) < new Date(breakEndEvent.event_time))
          .sort((a, b) => new Date(b.event_time).getTime() - new Date(a.event_time).getTime())[0];
        ...
        if (matchingBreakStart) {
          ...
          if (insertError) {
            console.error('Error creating break segment:', insertError);
            continue;
          }
      ...
      // Check for any remaining unprocessed clock-ins (might need to be paired with future clock-outs)
      const remainingClockIns = clockInEvents.filter(e => !processedInEvents.has(e.id));
      if (remainingClockIns.length > 0) {
        console.log(`Staff ${staffId} has ${remainingClockIns.length} unprocessed clock-ins`);
      }
      
      // Check for any remaining unprocessed break-starts (might need to be paired with future break-ends)
      const remainingBreakStarts = breakStartEvents.filter(e => !processedInEvents.has(e.id));
      if (remainingBreakStarts.length > 0) {
        console.log(`Staff ${staffId} has ${remainingBreakStarts.length} unprocessed break-starts`);
      }
      
      // Create open segments for any remaining unprocessed clock-ins or break-starts
      const pendingClockIn = remainingClockIns[0];
      const pendingBreakStart = remainingBreakStarts[0];
      
      if (pendingClockIn) {
        // Create an open work segment with null end_time and duration
        const { error: insertError } = await supabase
          .from('time_segments')
          .insert({
            staff_id: staffId,
        ...
        if (insertError) {
          console.error(`Error creating open work segment for staff ${staffId}:`, insertError);
        }
      }
      
      if (pendingBreakStart) {
        // Create an open break segment with null end_time and duration
        const { error: insertError } = await supabase
          .from('time_segments')
          .insert({
            staff_id: staffId,
        ...
        if (insertError) {
          console.error(`Error creating open break segment for staff ${staffId}:`, insertError);
        }
      }
    }
    
    // After processing all segments, regenerate the daily summaries
    await generateDailySummary(dateStr);
    
  } catch (error) {
    console.error('Error processing clock events into segments:', error);
  }
};
        ...
        if (unpaired.length > 0) {
          ...
          await supabase
            ...
            .insert({
              staff_id: staffId,
              date_worked: dateStr,
              clock_in_event_id: matchingEvent.id,
              clock_out_event_id: newEventId,
              start_time: matchingEvent.event_time,
              end_time: eventTime.toISOString(),
              segment_type: segmentType,
              break_type: segmentType === 'break' ? (matchingEvent.break_type || null) : null,
              duration_minutes: durationMins
            });
        }
      } catch (segmentError) {
