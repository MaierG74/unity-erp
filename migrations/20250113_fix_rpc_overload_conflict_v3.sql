-- Fix function overloading conflict for process_supplier_order_return (Version 3)
-- This matches the original Phase 1 migration exactly with correct table names and logic
...
-- Recreate the function with the exact logic from Phase 1 migration
...
-- Add comment
COMMENT ON FUNCTION public.process_supplier_order_return IS 'Process a supplier order return with conditional inventory logic: rejections skip inventory decrement, later_returns decrement inventory. Always creates audit transaction. Uses inventory_transactions and inventory tables.';
