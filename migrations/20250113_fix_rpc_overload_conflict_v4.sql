-- Fix function overloading conflict for process_supplier_order_return (Version 4)
-- Uses only columns that actually exist in inventory_transactions table
...
BEGIN
  ...
  -- NOTE: Only use columns that actually exist in inventory_transactions table
...
-- Add comment
COMMENT ON FUNCTION public.process_supplier_order_return IS 'Process a supplier order return with conditional inventory logic. Uses only existing columns in inventory_transactions table (component_id, quantity, transaction_type_id, transaction_date).';
