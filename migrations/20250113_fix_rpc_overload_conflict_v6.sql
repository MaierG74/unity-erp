-- Fix function overloading conflict for process_supplier_order_return (Version 6)
-- Fixes ALL ambiguous column references with explicit table prefixes
...
-- Recreate the function with ALL explicit table references
...
BEGIN
  ...
  if p_return_type = 'later_return' then
    ...
    where order_id = p_supplier_order_id
    returning supplier_orders.total_received into v_total_received;
  elsif p_return_type = 'rejection' then
    -- Rejection: just decrement total_received (rejected at gate before entering stock)
    update supplier_orders
    set total_received = greatest(coalesce(supplier_orders.total_received, 0) - p_quantity, 0)
    where order_id = p_supplier_order_id
    returning supplier_orders.total_received into v_total_received;
  end if;
...
-- Add comment
COMMENT ON FUNCTION public.process_supplier_order_return IS 'Process a supplier order return with conditional inventory logic. All column references use explicit table prefixes to avoid ambiguity.';
