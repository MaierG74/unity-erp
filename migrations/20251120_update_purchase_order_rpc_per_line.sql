-- Update the transactional helper to support per-line customer_order_id.
-- We keep the signature compatible but ignore the top-level customer_order_id if it's passed (or we can remove it, but let's keep it as optional for now to avoid breaking if called elsewhere, though we will update the form).
-- Actually, let's remove the top-level customer_order_id to be clean, as we are updating the only caller.

DROP FUNCTION IF EXISTS public.create_purchase_order_with_lines(integer, integer, jsonb, integer, timestamptz, text);
...
BEGIN
    ...
    WITH payload AS (
        SELECT
            ...
            customer_order_id -- Extract per-line customer_order_id
    ...
    SELECT
        so.order_id,
        payload.customer_order_id, -- Use the per-line customer_order_id
