-- Fix function overloading conflict for process_supplier_order_return (Version 2)
-- This corrects the schema references to use supplier_component_id
...
-- Recreate the function with the correct signature and schema
...
BEGIN
  ...
  -- Lock supplier order row for update (CORRECT: uses supplier_component_id)
  select
    so.order_id,
  ...
  if p_return_type = 'later_return' then
    ...
    select coalesce(quantity_in_stock, 0) into v_quantity_on_hand
    from component_inventory
    where component_id = v_component_id;
  end if;
...
-- Add comment
COMMENT ON FUNCTION public.process_supplier_order_return IS 'Process a supplier order return with conditional inventory logic: rejections skip inventory decrement, later_returns decrement inventory. Uses supplier_component_id to lookup component_id.';
