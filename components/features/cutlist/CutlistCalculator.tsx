...
export interface CutlistCalculatorProps {
  /** Initial data to populate the calculator with */
  initialData?: Partial<CutlistCalculatorData>;
  /** Called whenever calculator data changes (for auto-save) */
  ...
  saveMaterialDefaults?: boolean;
  /** localStorage key for parts persistence. null = no localStorage (default: null) */
  partsStorageKey?: string | null;
  /** localStorage key for optimization priority persistence. null = no localStorage (default: null) */
...
export const CutlistCalculator = React.forwardRef<CutlistCalculatorHandle, CutlistCalculatorProps>(function CutlistCalculator({
  ...
  const [backerBoards, setBackerBoards] = React.useState<BoardMaterial[]>(
    () => initialData?.backerBoards ?? []
  );
  const [edging, setEdging] = React.useState<EdgingMaterial[]>(
    () => initialData?.edging ?? []
  );
  const [kerf, setKerf] = React.useState(() => initialData?.kerf ?? DEFAULT_KERF);
  ...
  const [optimizationPriority, setOptimizationPriority] = React.useState<OptimizationPriority>(
  ...
  // Compute fractional sheets using per-sheet stock dimensions (supports mixed materials)
  const computeFractionalSheets = React.useCallback(
  ...
  const primarySheetsFractional = computeFractionalSheets(result);
  ...
  React.useEffect(() => {
    if (typeof window === 'undefined' || !partsStorageKey) {
      partsLoadedRef.current = true;
      return;
    }
    // Skip if initial data provided parts
    if (initialData?.parts && initialData.parts.length > 0) {
      partsLoadedRef.current = true;
      return;
    }
    try {
      const storedParts = window.localStorage.getItem(partsStorageKey);
      if (storedParts) {
  ...
  }, [primaryBoards, backerBoards, edging, kerf, shouldSaveDefaults]);

  // Persist parts to localStorage
  React.useEffect(() => {
    if (!partsStorageKey || !partsLoadedRef.current || typeof window === 'undefined') return;
    try {
      window.localStorage.setItem(partsStorageKey, JSON.stringify(parts));
    } catch (err) {
      console.warn('Failed to persist parts', err);
    }
  }, [parts, partsStorageKey]);

  // ============== Load optimization priority from localStorage ==============
  React.useEffect(() => {
    if (typeof window === 'undefined' || !optimizationStorageKey) return;
  ...
  React.useEffect(() => {
    if (!optimizationStorageKey || typeof window === 'undefined') return;
    if (!optimizationLoadedRef.current && !initialData?.optimizationPriority) return;
    try {
      window.localStorage.setItem(optimizationStorageKey, optimizationPriority);
    } catch (err) {
      console.warn('Failed to persist optimization priority', err);
    }
  }, [optimizationPriority, optimizationStorageKey, initialData?.optimizationPriority]);

  // ============== Notify parent of data changes ==============

  const dataChangeTimeoutRef = React.useRef<NodeJS.Timeout | null>(null);

  React.useEffect(() => {
    if (!onDataChange || !materialsLoadedRef.current || !partsLoadedRef.current) return;
  ...
  React.useEffect(() => {
    ...
    // Build per-edging-material breakdown for export
    const edgingByMaterial: EdgingSummaryEntry[] = [];
    const edgingLookup = new Map<string, typeof edging[0]>();
...
export type { CompactPart, BoardMaterial, EdgingMaterial } from '@/components/features/cutlist/primitives';
export type { CutlistSummary, SheetBillingOverride, CutlistLineRefs, CutlistLineInput } from '@/lib/cutlist/types';
