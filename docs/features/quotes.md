...
The quote line's base price remains under estimator control. New product rows pull the product's saved Standard price-list `selling_price` into `quote_items.unit_price` as the initial base price, while still allowing the estimator to edit the quote line price afterward. This applies to both the default snapshot-based product row and the opt-in legacy exploded costing-cluster row. Snapshot-based product rows expose a per-BOM-row swap action that can choose an alternative same-category component or remove the component, with a zero, positive, or negative commercial surcharge. Applying a swap updates `quote_items.bom_snapshot` and `quote_items.surcharge_total` together; the database quote totals trigger then recomputes subtotal and grand total from the base item total plus surcharge.
