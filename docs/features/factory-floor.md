...
Sections are **configurable** — new sections can be added to `factory_sections` and linked to a `job_categories` entry. Jobs are routed to sections automatically based on their category.
...
- **Link to a job category** (select from the Category dropdown — jobs with that category automatically appear in the section)
