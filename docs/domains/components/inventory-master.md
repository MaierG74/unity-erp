...
- Order detail component availability depends on the customer order having a real active `orders.status_id`. Production migration `20260610072133_order_status_default_hotfix` sets a `New` default and before-insert trigger so new statusless orders do not drop out of `get_detailed_component_status`; PO2036/PO21914 were repaired during the hotfix.
