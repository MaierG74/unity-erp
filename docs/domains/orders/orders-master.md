...
- Detail page `app/orders/[orderId]/page.tsx` loads header (`orders` with FK-hinted `order_statuses`, `customers`, and `quotes`) plus `order_details(product:products)`. Keep those embeds explicitly hinted (`orders_status_id_fkey`, `orders_customer_id_fkey`, `orders_quote_id_fkey`) so PostgREST relationship ambiguity cannot blank the whole order page. For configurable products, extend the line editor to surface option selectors sourced from attached **Option Sets** (global + product overlays), persist `selected_options`, and call the shared resolver so FG reservations and purchasing respect the chosen configuration even when an order is created directly (no quote).
...
- The Add Products dialog submits selected products directly to `order_details`. Operators configure BOM swaps, cutlist material, and component readiness from the resulting order line rather than through a pre-add configuration modal.
...
- In the order detail Cutting Plan tab, saved v2 plans split Material Breakdown by independent material nests keyed by `(kind, sheet_thickness_mm, material_id)`. Same primary material and thickness consolidates across backed/non-backed products, backer nests render as their own "Backer" rows, and each row exposes one cutter cut-list PDF once the generated plan is current and part labels are loaded. Legacy v1 plans render only the regenerate banner until overwritten. See [`../../plans/2026-05-05-cutter-diagram-print.md`](../../plans/2026-05-05-cutter-diagram-print.md).
...
- The Quality dropdown next to **Generate Cutting Plan** maps to a real packer choice (POL-93). **Fast** runs the strip packer with a 2-second budget — sub-second on typical inputs; pick this for quick iteration. **Balanced** runs the guillotine packer with the same 2-second budget — tighter nests on most layouts at no perceptible latency cost. **Quality** runs simulated annealing with a 30-second budget; the Generate button reports iteration / elapsed / best-score progress while running, and a Cancel button (deep mode only) aborts the run cleanly without persisting a partial plan. The persisted `optimization_quality` field on `orders.cutting_plan` reflects the dropdown selection at generation time. The standalone `/cutlist` calculator's behaviour is unchanged; this wiring is purely on the order-level tab.
...
- **Stock Issuance** (✅ Implemented January 2025):
  ...
  - Inventory availability is fetched from the live `inventory.quantity_on_hand` snapshot. The Issue Stock inventory query uses a short freshness window and refetches on browser-window focus so operators who adjust stock in another window can return to the order and see updated availability without a full reload. The card header also exposes **Refresh availability**, which refetches inventory, issuance history, component requirements/status, and order smart-count queries in place while preserving the current tab, expanded products, selections, and typed issue quantities.
...
- Product-line deletion also clears BOM swap exception rows for the deleted `order_detail_id` after the production/component activity guards pass, so stale exception queue rows do not block removing an otherwise safe test or draft line.
