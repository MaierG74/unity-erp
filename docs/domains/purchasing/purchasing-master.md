...
- **Create PO (manual):** Multi-line form to select components, pick supplier per-line, and set quantities/notes.
  ...
  - Component picker: `components/features/purchasing/ComponentSearchModal.tsx` searches through `app/api/purchasing/component-picker/route.ts`, which enforces the Purchasing module entitlement, scopes reads to the active organization, and returns bounded results (default 50) instead of preloading the full component catalog into the browser. The picker is optimized for known-item entry: exact/prefix code matches are ranked first, supplier/category filters are server-side, and operators should keep typing to narrow broad searches. Migration `20260524103000_po_component_picker_search_indexes.sql` adds the supporting scoped/trigram indexes for large catalogs.
...
- Payment lifecycle (Slice 1 — `awaiting_payment → awaiting_pop → closed`, spec `docs/projects/2026-07-01-cash-supplier-payment-lifecycle-completion.md`):
  - **Record payment**: `RecordPaymentDialog` (Awaiting-payment card button or POP file drop) → optional POP upload → `record_payment` RPC (amount/date/method eft|cash|card/reference; `paid_at`; → `awaiting_pop`).
  - **Sign-off**: `sign_off_payment` RPC, gated to active org role owner/admin (`is_org_payment_authoriser`); board shows the Sign off button only when the API returns `caller_can_authorise`; sign-off locks the payment (amend via `reopen_payment`, same role gate, note required).
  - **POP out**: "Send POP to supplier" (cards with a POP file) → `app/api/send-pop-email/route.ts` (module-gated, RLS row checks, Resend with the POP attached, `purchase_order_emails` `email_type='po_pop_send'`) then `mark_pop_sent`; or "Mark sent" directly (note required when no POP file). `mark_pop_sent` also accepts a late POP attachment. Closed cards leave the board.
...
- Cash-supplier payment (Slice 1): on the Awaiting-payment card, Record payment (amount prefills from the invoice) → card moves to Awaiting POP with an “Awaiting sign-off” chip. As a staff-role user the Sign off button must be absent (RPC refuses with `insufficient_privilege`); as owner/admin it appears — click and the chip flips to “Signed off”. Send POP (card with a POP file) emails the supplier’s primary address with the file attached, logs `po_pop_send`, and closes the card; Mark sent without a POP file demands a note. New invoice/POP files must land in `finance-docs` (direct client `storage.download` refused; preview works via the access-url route).
