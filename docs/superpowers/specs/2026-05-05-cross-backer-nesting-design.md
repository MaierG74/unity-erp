...
status: v5 — SIGNED OFF (GPT-5.5 Pro round 4)
date: 2026-05-05
...
review-rounds: r1 — 1 BLOCKER + 6 MAJORs + 4 MINORs (→ v2); r2 — 2 BLOCKERs + 5 MAJORs + 2 MINORs (→ v3); r3 — 0 BLOCKERs + 2 MAJORs + 2 MINORs (→ v4); r4 — 0 BLOCKERs + 0 MAJORs + 1 MINOR clarified in v5 — **SIGNED OFF FOR IMPLEMENTATION**
verdict: "Sign off as-is — Codex implementation can proceed."
totals: 3 BLOCKERs + 13 MAJORs + 9 MINORs across 4 rounds
---

# Cross-color cutting-plan nesting (POL-94)

## What changed in v5 (post round-4 sign-off)

Round-4 returned the sign-off verdict with one MINOR — implementation-only clarification on the "component not found in DB query" path (separate from the three `BACKER_THICKNESS_INVALID` reason values). Single-line clarification in the aggregate-route file-by-file row; no contract change.

## What changed in v4 (post round-3)

Round-3 returned 0 BLOCKERs + 2 MAJORs + 2 MINORs — all stale-v2-wording cleanups within file-by-file rows that hadn't been updated when DP5 shifted to plausibility-range validation. No new architectural work; the spec was already implementation-ready in substance.

| # | Severity | Issue | Where addressed in v4 |
|---|---|---|---|
| 1 | MAJOR | DP5 v3 contract correct but file-by-file rows for aggregator / material-regroup / aggregate-route still describe the v2 "known-thickness set" | Replaced "known-thickness set" wording with explicit category + plausibility-range validation on all three rows; aggregate-route query now selects `(component_id, description, category_id)` |
| 2 | MAJOR | DP5 error-shape only listed `'null' \| 'out_of_range'`; missing `'wrong_category'` reason that exists in Grouping rule §3 | Normalized error shape across DP5 + Grouping rule + file-by-file: `BACKER_THICKNESS_INVALID` with `invalid: Array<{component_id, parsed_value, reason: 'wrong_category' \| 'null' \| 'out_of_range'}>` |
| 3 | MINOR | Phase 1 said "known-thickness sanity check" | Renamed to "category + plausibility-range validation" |
| 4 | MINOR | `-both` AC12 covered placement + sheet count parity but not cost parity | Extended AC12 to also assert `total_nested_cost` parity (cheap, since AC11 pins cost formula correctness anyway) |
...
| 7 | MAJOR | Parser only handles null, not silent-wrong-parses | DP5 sanity-check (later replaced in v3 with category + plausibility-range validation), AC10, Risks: "silent-wrong-parse capable" |
...
| `lib/orders/cutting-plan-aggregate.ts` | Rewrite `resolveAggregatedGroups`: emit primary group for every part, plus a band-edges-zeroed backer copy (id `${part.id}::backer`) for every `-backer` part. Add a `backerLookup: Map<number, { thickness_mm: number; category_id: number }>` parameter (caller fetches + parses + validates upstream — see aggregate route row). The aggregator itself trusts the lookup contents are valid; if any referenced backer ID is absent from the lookup, return `{ ok: false, error: 'BACKER_THICKNESS_INVALID', invalid: Array<{component_id, parsed_value: number \| null, reason: 'wrong_category' \| 'null' \| 'out_of_range'}> }`. The error shape carries per-component reason so the UI can surface a precise message. |
| `lib/orders/material-regroup.ts` | Same dual-emit logic with band_edges zeroing on the backer copy and the `::backer` id namespacing for the post-assignment regroup path. Same `backerLookup` parameter — populated client-side from `useBackerComponents()` data (which carries `category_id` and parsed thickness). Client applies the same three checks (category + non-null + plausibility range) before constructing the lookup; on failure surfaces the toast and refuses to regroup. |
| `app/api/orders/[orderId]/cutting-plan/aggregate/route.ts` | Resolve backer component metadata before calling the aggregator. Server collects all `effective_backer_id` and snapshot `backer_material_id` values, queries `SELECT component_id, description, category_id FROM components WHERE component_id IN (<ids>) AND org_id = <org>`. **Missing-from-query path** (a referenced backer ID didn't return a row, e.g. deleted or org mismatch): return `{ ok: false, error: 'BACKER_COMPONENT_NOT_FOUND', missing_component_ids: number[] }` — distinct from the validation path so the UI surfaces "Backer component not found — reload the page or pick a different backer" rather than the description-fix prompt. **Validation path** (rows returned but invalid): apply `parseThicknessFromDescription()` to each description, then check each: (1) `category_id ∈ BACKER_CATEGORY_IDS = [75, 3]`, (2) parsed value non-null, (3) `0.5 ≤ thickness ≤ 50`. On any failure return `BACKER_THICKNESS_INVALID` with `invalid: Array<{component_id, parsed_value, reason: 'wrong_category' \| 'null' \| 'out_of_range'}>` directly — do not call into the aggregator. On full success, build `backerLookup` and call the aggregator. The enriched groups (with `sheet_thickness_mm` resolved) are returned in the response. |
...
1. **Phase 1 — Type shape + aggregator.** Update `cutting-plan-types.ts` (discriminated union, `version: 2`, `stale_reason` field, `DisplayPlanState` union), `cutting-plan-aggregate.ts`, `material-regroup.ts`. Add backer-thickness lookup with **category + plausibility-range validation** (`category_id ∈ BACKER_CATEGORY_IDS`, non-null parse, `0.5..50` mm). Unit tests covering: (a) the worked example bug fixture, (b) multi-primary shared-backer, (c) per-line POL-83 override fixture (two lines of the same product resolve to different effective primary materials), (d) band_edges-zeroed assertion on backer copies, (e) `-both` no-op pass-through, (f) validation rejection cases (`"316mm offcut"` → out_of_range, `"Super White Backer Board"` → null, plywood/edging component → wrong_category, `"1.6mm Formica"` → valid).
...
12. **`-both` regression (new in v3, extended in v4)** — fixture with one `32mm-both` product (e.g. test order 401's shape): generating the v2 plan produces (a) the same number of placements per visible part (2 — encoded upstream in `product_cutlist_groups.parts`), (b) the same primary sheet count, AND (c) the same `total_nested_cost` as today's behavior. All three asserted, since AC11 already pins the cost formula. Together they confirm the aggregator's pass-through is truly a no-op for `-both`.
