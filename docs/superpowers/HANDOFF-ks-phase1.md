...
- **Collector per-instance identity (Codex stop-gate, 2026-06-16) — resolve before/while building Task 7.** `collectOrientedParts` aggregates instances into qty-rows keyed by name+dims+colour, discarding per-instance position. Consequences: Unity emits **Left Side / Right Side** as SEPARATE rows with mirror banding (Left `{right:true}`, Right `{left:true}`), but KS produces ONE aggregated `Side …` row `{right:true}` qty 2 (meterage-equivalent, not row-identical); and deterministic IDs must use a STABLE key, not Map/traversal order. **Decision before Task 7:** (i) carry per-instance positions → split Left/Right with mirror banding + position-based stable IDs (exact Unity per-row parity); or (ii) keep aggregated qty-rows + IDs by a stable sort (role+dims+name) accepting meterage-equivalent sides. Rec: (i) if the cut diagram needs per-side parity, (ii) if costing/meterage parity suffices for the demo.

**First action:** read the two plan docs, run `npx vitest run` to confirm the 34-green baseline, resolve the collector decision above, then start **Task 6** with the per-task loop (TDD → Codex `xhigh` review → commit).
