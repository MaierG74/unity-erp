...
1. **Working tree sanity.** Run `git status --short` (expect empty) and `git branch --show-current` (expect `codex/local-job-card-drawings-spec`). If either is wrong, stop. Run `git log --oneline -5` and confirm the top commit is `7136d35 plan(audit-fix): Phase 2 ExecPlan for job-card drawings audit findings` on top of `0129d92 docs(audit): job card drawings — issuance, PDF render, data flow`.
