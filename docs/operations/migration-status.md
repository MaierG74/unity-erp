...
- Verification notes:
  ...
  - Picking-list stock reservation (2026-06-04, Claude Code) — branch `codex/local-picking-reservation`:
    ...
    3. SQL functional smoke (single self-rolling-back txn → **zero residual**, verified): pick 16 → reserved 16 / on-hand 16; 4×4 batch issue → on-hand 16→12→8→4→0, reserved 16→12→8→4→0, status partially_issued×3 then issued, item.quantity_issued=16, four `stock_issuances` linked via `pending_item_id`; cancel released the remainder. **Reservation invariant returned ZERO violations after every step.**
    4. `get_advisors` (security + performance): no NEW RLS or mutable-search-path findings attributable to the migration (the 3 new RPCs all pin `search_path`; the 3 pre-existing `LANGUAGE sql` functions keep their mutable search_path unchanged). The 2 `security_definer_view` ERRORs were fixed (item 2). `idx_stock_issuances_pending_item` present. The 3 new `authenticated_security_definer_function_executable` WARNs are by-design (org-checked RPCs; matches the project-wide pattern of 63).
    5. Frontend (this branch) is NOT yet deployed. Applying the migration makes picking reserve stock DB-side for ALL environments immediately; the DB blocks over-picks regardless of which frontend version an env runs, but the UI only *shows* held availability where the new frontend has shipped.
  ...
  - Cash-supplier invoice tracking — Part Two (2026-07-01, Claude Code, branch `codex/local-cash-supplier-tracking`):
    ...
    4. `record_invoice_org_hardening` (20260701205117): pre-merge review fix for cross-org reference poisoning (RLS checked only the row's own `org_id`, never the referenced PO/invoice's org). Adds BEFORE INSERT/UPDATE triggers `po_invoices_enforce_org` (org must match the PO's org; invoice/pop attachment ids must belong to the same PO) and `po_signoff_activity_enforce_org` (org must match the invoice's org). Re-creates `record_invoice` with: `FOR UPDATE` lock on the `purchase_orders` row (serializes double-submits), `org_id = v_org_id` filter on the row-reuse SELECT, and `p_attachment_id`-belongs-to-PO validation. Verified via rolled-back `DO`-block: cross-org poisoned insert blocked (`check_violation`), same-org insert accepted; advisors unchanged.

  - Cash-supplier payment lifecycle — Slice 1 (2026-07-01, Claude Code, branch `codex/local-cash-payment-lifecycle`):
    1. `payment_lifecycle_rpcs` (20260701214023): `record_payment` / `sign_off_payment` / `mark_pop_sent` / `reopen_payment` + `is_org_payment_authoriser` helper. All SECURITY DEFINER, hardening pattern from 20260701205117 (PO row lock, org checks, attachment ownership, both audit trails). Sign-off/reopen gated to active org role owner/admin. State machine: awaiting_payment → awaiting_pop → closed; reopen from awaiting_pop/closed.
    2. `finance_docs_and_gating` (20260701214107): private `finance-docs` bucket + INSERT-only org-prefixed storage policy (no authenticated SELECT — reads server-signed after RLS row check); `po_attachments_enforce_org` trigger (org_id derived from parent PO) + org_id column DEFAULT dropped (closes the default-org stamping gap); `storage_bucket`/`storage_path` columns; `purchase_order_emails.email_type` CHECK extended (`po_balance_close` drift + `po_pop_send` + `po_payment_reminder`); `finance` module seeded + QButton entitled.
    3. Verified: rolled-back trigger test (bogus org overwritten with PO org); bucket private, policy counts correct; 4 RPCs present; entitlement enabled. Full advisor pass with the slice review.
    4. `payment_lifecycle_hardening_r2` (20260701221529): review-fleet fixes — lifecycle writes now RPC-only (dropped INSERT/UPDATE/DELETE policies on `purchase_order_invoices` + INSERT policy on `po_payment_signoff_activity`; closes the staff→PostgREST sign-off/close bypass); attachment trigger forbids cross-PO moves and validates `finance-docs` `storage_path` against the PO org prefix (blocks cross-org object signing); `is_org_payment_authoriser` respects `banned_until`; all four RPCs lock the parent PO first (consistent lock ordering vs `record_invoice`). Verified live: write-policy counts 0; spoofed-path insert + cross-PO move blocked (rolled-back DO test); staff-token direct UPDATE/DELETE refused (0 rows).
