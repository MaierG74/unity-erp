...
- **Balance-closure notices:** `POST /api/send-po-balance-closure-email` sends a scoped notice after a partially received line's outstanding balance is closed. It asks the supplier to cancel/short-close only the selected outstanding quantity, includes any remaining outstanding quantity, and logs the result against the supplier-order line.
- **Typed email logs:** `purchase_order_emails.email_type` classifies PO mail as `po_send`, `po_cancel`, `po_line_cancel`, `po_balance_close`, `po_follow_up`, `po_pop_send` (proof-of-payment to a cash supplier, `emails/pop-email.tsx` via `app/api/send-pop-email/route.ts`), or `po_payment_reminder` (payment-escalation nudges from `supabase/functions/process-payment-escalations`; cron sequence in [`cash-escalation-cron.md`](cash-escalation-cron.md)); `supplier_order_id` stores line-level linkage where applicable. The DB CHECK constraint was aligned with this full list in migration `20260701214107` (it previously lacked `po_balance_close`).
