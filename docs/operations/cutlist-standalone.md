...
- Share the same core calculator behavior used by the dedicated Quote and Product cutlist pages so results stay aligned across Unity.
...
- **Layout & Inputs** (left column, wrapped in a card)
  - Embeds the shared `CutlistCalculator` with the full materials, parts, preview, and costing experience.
...
## Data Flow
- `CutlistCalculator` emits `CutlistSummary` payloads whenever results update; the payload contains `materials[]` entries with sheet, edging, and backer costs per material.
...
## Differences vs. Quote Cutlist Page
...
- The material palette and per-part material column are unique to this standalone page; the quote-specific page adds quote export behavior and quote-line persistence on top of the same main calculator.
