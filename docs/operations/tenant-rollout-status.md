...
- Timekeeper compatibility hotfix chain applied on 2026-02-25 (migrations: `timekeeper_anon_read_hotfix_qbutton`, `timekeeper_anon_insert_policy_fix`, `timekeeper_anon_policy_uuid_lock_fix`, `timekeeper_trigger_security_definer_fix`): restored scanner read/insert for the external non-authenticated Timekeeper app while keeping rules pinned to active `Qbutton` staff and preserving authenticated org-scoped policies.
